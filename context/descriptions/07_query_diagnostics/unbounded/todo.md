...
names SELECT-only columns from their user-written projection aliases. What remains are two
pending tasks plus a deferred item blocked on unrelated expressivity work.

---

## Pending

- **Cap and dedupe the categorical rule list on narrow inputs.** `CharacterizeEscape`
  (`decide_diagnostic.cpp:327`) emits *every* group clearing the escape-rate threshold, with no
  limit, `; `-joined into one cell. On a small input this floods: the candidate filter is
  `2 ≤ distinct ≤ max(min_categories, ratio × N)` with `min_categories` = 20
  (`physical_decide.cpp:2666`), so on an N-row table with N ≲ 20 *every* column is categorical,
  and every singleton group holding an escaper scores rate 1.0. Measured on the paper's Figure 1
  data (3 joined rows, 9 columns, 2 escaping rows): **12 rules in one cell**, including
  ...
  **Change:** (a) drop rules whose escaping-instance set is a subset of an already-emitted
  rule's set, so perfectly-correlated columns collapse to one representative; (b) cap the
  surviving list (proposed: 3, pragma-adjustable) ordered by instances covered, then rate, then
  the existing column/value tie-break.
  ...
  **Test:** extend `test/decide/tests/test_query_diagnostics_escaping_instances.py` with a
  narrow-table case (≤ 5 rows, many columns) asserting a bounded rule count and that the
  query-referenced column survives; keep an existing wide-table case green to show the cap does
  not change large-input behavior.
  **Done file:** merge into `done.md` → "`affected_rows` / `affected_entities`". Discovered
...
- **Scope the prescribed bound to the escaping rows.** (Not paper-blocking: §4's unbounded
  ...
  single global statement (`x <= <cap>`) carried in the summary, while `affected_rows` may
  report that only a slice escapes (`2 of 3 rows where priority = 'standard'`). The finding
  is scoped and the repair is not, so the user must invert the characterization themselves to
  find the edit — and the global cap also constrains rows that were already bounded.
  ...
  **Pointers:** rule computation `CharacterizeEscape`; formatter `BuildUnboundedDiagnostic`
  (`decide_diagnostic_engines.cpp`); the coverage flag has to come out of `CharacterizeEscape`
  alongside the rules, since the formatter cannot recover it from the rendered string.
  ...
  **Done file:** merge into `done.md` under "The output relation" + the `affected_rows`
  section. Discovered 2026-08-04 while checking the paper's §4 unbounded example, whose
