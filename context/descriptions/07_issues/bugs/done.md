...
**Fix/lesson**: The bare form is a genuine LALR(1) shift/reduce ambiguity — after `PER region` with lookahead `,`, the parser cannot tell "another PER column" from "next constraint" (a constraint item can be a bare `a_expr`). This is *why* the parenthesized form exists; parentheses are the disambiguator (same reason SQL parenthesizes row-value tuples). Decided (2026-07-02) not to add the bare form. Instead specialized the top-level `COLUMN_REF` case in `decide_constraints_binder.cpp` (a lone column is never a valid `SUCH THAT` constraint) to emit an actionable hint: *"'yr' is not a valid SUCH THAT constraint on its own. To group a constraint by multiple columns, parenthesize them: PER (col1, col2)."* Documented the parenthesization requirement in `syntax_reference.md` §7.3. **Watch for**: before "fixing a binder gap," check the grammar — a symptom that looks like an incomplete binder can be a deliberately unambiguous syntax boundary.
