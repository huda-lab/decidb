...
## MIN/MAX linking rows named the same solver column twice

**Broke**: Any `MIN`/`MAX` over decision variables whose per-row expression was not a single product term failed before reaching the solver — `Failed to add constraint to Gurobi: Problem adding constraints`, or `Failed to pass model to HiGHS: status -1`. Three distinct triggers, all one root cause: a multi-term row expression (`MIN((cost + 1) * x)`, which distributes into `cost*x + 1*x`, or `MIN(cost * x + x)` written out); an entity-scoped or `SCALAR` variable inside a PER group, where a *single* term suffices because one column spans every row of the group; and a bare constant (`MIN(cost * x + 5)`). Affected the flat objective path, both PER paths, and both composed paths, in easy and hard directions alike. Loud failure throughout — no wrong answers were produced.

**Cause**: The linking rows were built by pushing one `(column, coefficient)` pair **per term**, straight from `saved_obj_var_indices` / `inner_terms`. Those arrays are indexed by term, not by variable (`physical_decide.cpp` pushes one entry per term unconditionally), so the same solver column could enter one row repeatedly — and a repeated column index in a single constraint is rejected outright by both backends. The constant case was a second defect in the same loops: `Term::variable_index` is `INVALID_INDEX` for constants, and the flat/PER sites handed that to `VarIndexer::Get()`, which indexes `var_scope[INVALID_INDEX]` — an out-of-bounds vector read, undefined behavior in release, yielding a garbage column. Neither defect reached constraints, because the model builder deduplicates there (`ilp_model_builder.cpp`, the `seen` set), nor plain `SUM` objectives, whose coefficients accumulate with `+=`.

**Fix/lesson**: One `MinMaxLinkRow` accumulator plus two `Add*RowTerms` lambdas in `physical_decide.cpp`, used by all nine emission sites: coefficients sum per solver column, columns that cancel are dropped, and constants collect into an offset the caller folds into the row's bound (`z <= expr + k` → `z - expr <= k`). Columns keep first-appearance order rather than hash order, so the same query always hands the solver the same matrix. The composed path already folded constants correctly and was the reference for that half; the accumulate-by-column idiom already existed twice in the file (`obj_coef_accum`, `outer_accum`). **Watch for**: any array in this operator named `*_var_indices` is *term*-indexed — the same variable legitimately appears more than once, and `INVALID_INDEX` legitimately appears at all. Never feed one to `VarIndexer::Get()` or to a `RawConstraint` without accumulating first. Note also that duplicate columns are fatal in a *constraint row* but harmless in the *objective* vector, which is why `SUM` objectives never surfaced this. Tests: `test/decide/tests/test_min_max_multiterm.py` — seven oracle-verified cases over flat / composed / PER × easy / hard × row- and entity-scoped, each pinning the objective value so a row that is merely accepted but mis-linearized still fails. Found 2026-08-09; the original report covered only the multi-term flat-objective trigger, and the PER entity-scope and constant triggers turned up while sizing it.

## Table qualifiers on data columns were dropped by the symbolic round trip

**Broke**: A *correctly qualified* data column used as a coefficient inside `SUCH THAT` or `MAXIMIZE` was rejected as ambiguous, with an error naming the exact syntax the user had already written: `SUM(x * t1.w)` over `FROM t1 JOIN t2` (both exposing `w`) → `Ambiguous reference to column name "w" (use: "t1.w" or "t2.w")`. The same references bound fine in the `SELECT` list, and the same query without a `DECIDE` block was valid SQL.

**Cause**: A `Symbolic` symbol carries only a name. `ToSymbolicRecursive` stored `colref.GetColumnName()` — the last path component only — and `FromSymbolic` rebuilt a single-part `ColumnRefExpression(name)`. The qualifier was never stored anywhere, so the binder was handed a bare `w` it had never been given. Two consequences: same-named columns from different tables collapsed into *one* symbol inside the algebra, and the rebuilt reference could not resolve. Failure was always a hard error, never a wrong number — losing a qualifier can make a name ambiguous or unresolvable, but cannot silently redirect it to a different column.

**Fix/lesson**: Added `SymbolicTranslationContext::column_map`, keyed by the column's full dotted path (lowercased), holding a copy of the original reference and restored verbatim in `FromSymbolic` — the same opaque-placeholder pattern `abs_map`/`data_map`/`subquery_map` already use. Keying by full path keeps distinct columns distinct in the algebra while letting repeated references to one column still fold. **Decision variables are deliberately excluded** and keep canonicalizing to their unqualified name: `bind_select_node.cpp:1132` always registers that form (the qualified spelling is an alias at `:1148`) and `:1125` rejects two variables sharing a base name, so the unqualified name always resolves — routing them through `column_map` would instead split `keepS` and `S.keepS` into two symbols and break the `decide_variables.count(name)` classification in `SymbolicContainsDecideVariable` / `CollectDecideFactors`. **Watch for**: restore-by-copy rather than reconstructing identifiers from a joined string — `column_names` is a stack of arbitrary depth (`catalog.schema.table.column`) and quoted identifiers may contain a dot, so splitting on `.` is lossy in both directions. More generally, any lossy round trip through the symbolic layer shows up as a binder error about something the user never typed; when an error quotes syntax back that matches what was written, suspect the rebuild, not the input. Tests: `test/decide/tests/test_qualified_columns.py` (self-join on `nation`, since TPC-H prefixes every column; verified to fail on the pre-fix build). Found 2026-07-26 checking whether a CTE could express the tuple-count objective of paper Example 2.1.

## Build broke on newer Xcode Command Line Tools: `TEnumIterator` has no `operator==`

**Broke**: `make release` failed compiling the generated `third_party/parquet/parquet_types.cpp` — `invalid operands to binary expression ('duckdb_apache::thrift::TEnumIterator' and 'duckdb_apache::thrift::TEnumIterator')` from inside libc++'s `__tree:1126`. Unrelated to any DeciDB source; the tree blocked all builds.

**Fix/lesson**: `TEnumIterator` (`third_party/thrift/thrift/Thrift.h:54`) defined only `operator!=`. Older libc++ implemented `std::__tree::__insert_range_unique` with `__first != __last`; the libc++ in newer Command Line Tools uses `__first == __last`, which the iterator does not provide, so `std::map`'s range constructor stopped compiling. Added a matching `operator==` mirroring the existing `operator!=` (const, so it also binds to const iterators). **Watch for**: this is a vendored-thrift patch, so an upstream third_party refresh will drop it and the build will break again the same way. A build error pointing only at SDK headers with a vendored type in the operands is a missing operator on the vendored type, not an SDK problem. Found 2026-07-26.

## A single convex quadratic *constraint* stalled the QCQP solve (and could error on a usable solution)

**Broke**: Adding one convex quadratic constraint (`SUM(POWER(qty,2)) <= K`) to an otherwise fast continuous QP made the Gurobi solve several-to-160× slower and scale superlinearly in rows (Q5: 50K rows 1.7s, 100K rows 32s). On some data the barrier stopped at Gurobi "sub-optimal termination" (`GRB_SUBOPTIMAL`, status 13), which the backend's status switch did not handle — it fell to `SolverStatus::OTHER` and threw `"DECIDE optimization failed (solver status 13)"`, **discarding a perfectly good feasible solution**. The quadratic *objective* alone was nearly free (0.1–0.2s); only the quadratic *constraint* was slow.

**Cause**: A quadratic constraint flips Gurobi out of its QP solver into SOCP mode, reformulating *both* the quadratic objective and the constraint into second-order cones (~200K rows for 100K vars). The default barrier reaches the optimum early (iter ~16, 2s) then oscillates in the dual for ~60 more iterations (to 22s), stalling and sometimes stopping sub-optimal. The original hypothesis in `todo.md` — an expensive *crossover* — was empirically disproven: `Crossover=0` had zero effect; the whole cost is inside the barrier. Probed with `gurobi_cl` on a faithful diagonal QCQP LP: `NumericFocus=2` → true OPTIMAL in 5.8s; `BarQCPConvTol=1e-3` → OPTIMAL in 2.2s (looser tol); `Crossover`/`BarHomogeneous`/`Method` → no help.

**Fix/lesson**: (1) Set `NumericFocus=2` on the Gurobi env **only when the model carries quadratic constraints** (`gurobi_solver.cpp` `Load`) — the barrier converges to a true optimum instead of stalling (Q5 large 32s → 9s clean OPTIMAL), and the fast pure-QP-objective path (no SOC reformulation) is untouched. Correctness-first: keeps full tolerance, doesn't silently loosen the answer. (2) Handle `GRB_SUBOPTIMAL` as a first-class feasible-but-unproven status: new `SolverStatus::SUBOPTIMAL` carries the incumbent (like the TIME_LIMIT incumbent), routes to the SOLVED success terminal (`decide_router.cpp`), and the operator delivers the rows with a plain "not proven best" stderr caveat (`physical_decide.cpp`) instead of erroring. Gurobi-only throughout — HiGHS rejects quadratic constraints upstream. **Watch for**: a quadratic *constraint* (not objective) is what triggers Gurobi's SOC reformulation and the slow barrier — the QP objective alone stays on the fast path. Diagnose barrier stalls empirically (watch the tail dual residual oscillate) before blaming crossover. Any solver "sub-optimal" terminal still carries a usable solution — return it, don't discard it. Pointers: `gurobi_solver.cpp` (NumericFocus gate; `GRB_SUBOPTIMAL` case), `solver_result.hpp` (`SolverStatus::SUBOPTIMAL`), `decide_router.cpp`, `ilp_solver.cpp` (`ThrowDecideSolveError` exhaustiveness). Benchmark: Q5 (`benchmark/decide/queries/q5_qp_qcqp.sql`). Found 2026-07-24 rebuilding the DECIDE benchmark suite; Q5 was the anomalously slow query.

## Data-only aggregate as a coefficient (`SUM(avg(p) * x)`) silently miscomputed

**Broke**: A data-only aggregate (`avg(p)`/`min(p)`/`max(p)` over table columns, no DECIDE variable) used as a coefficient multiplier returned a wrong optimum with no error. `SUM(avg(p) * x) <= 3` (avg(p)=15, should force `x=[0,0]`) returned `x=[1,1]`. Cause: `NormalizeDecideConstraints` round-trips the constraint through SymEngine, where `avg(p)` is represented as `__AVG__ * p`; so `avg(p) * x` becomes `__AVG__ * p * x` and `FromSymbolic` reconstructs it as `avg(p * x)` — a *different* constraint (the variable distributed into the aggregate). Validation then passes because the mangled form "references x", and the coefficient is wrong. `sum(p) * x` reconstructs differently (stays `sum(p) * x`) and was already rejected, so the bug looked AVG-specific but is really any-aggregate-as-coefficient; only `sum` happened to be caught.

**Fix/lesson**: Reject the pattern in `ValidateDecideNoNonLinearScalar` (`decide_binder.cpp`), which runs *before* normalization — at any `*` node that both contains a decision variable and contains a data-only aggregate (`ContainsDataOnlyAggregate`), throw a friendly "an aggregate over table columns cannot multiply a decision variable; pre-compute it as a scalar or move it to the RHS". Now all of avg/min/max/sum reject uniformly. Data-only aggregate *RHS* (`SUM(x) <= AVG(p)`) and genuine DECIDE aggregates (`AVG(x * p)`) are unaffected — the guard requires a decide var *inside the product*. **Watch for**: a SymEngine (or any CAS) round-trip that models an aggregate as `MARKER * body` will silently distribute an outer factor into the body — validate structural constraints on the *pre-normalization* parse tree, not after. Tests: `test_error_binder.py::TestBinderErrors::test_data_only_aggregate_coefficient_rejected` (all four aggregates) + `_rhs_still_allowed` (guards against over-rejecting). Found prioritizing the two AVG bugs after the diagnostics-label fix.
...
**Fix/lesson**: Three parts, all keyed on "every user limit must be carried by a slackable row, the column box only the intrinsic domain." (A) In the `DiagnosisTerminal::INFEASIBLE` arm, after re-emitting absorbed bounds as rows, reset each non-BOOLEAN decide column's implied tightenings back to intrinsic (`col_lower>0 → 0`, `col_upper → +inf`) — safe because every implied tightening has a backing row that still enforces it. (B) Add `SolverInput::tolerate_infeasible_bounds` (set when diagnosis is armed): `Build` keeps the inverted box instead of throwing, and `SolveModel` short-circuits to INFEASIBLE **without handing the inverted box to the backend** (HiGHS hard-rejects `lb>ub` at load and poisons the session; Gurobi tolerates it — solver-agnostic care). (C) A user bound that contradicts the *intrinsic* domain (`x <= -1` on non-negative REAL, `x >= 2`/`x = 2` on BOOLEAN) is a deterministic type error, **not** an elastic edit: a post-absorption check in the sink constructor throws a precise static message (`"x >= 2 cannot hold because x is BOOLEAN (0 or 1)"`). **Watch for**: `op.is_boolean_var[var]` is the reliable boolean signal for this reset, checked *before* the generic `is_binary` skip branch — `is_binary[col]` now also reads `true` for a BOOLEAN-domain variable (the domain round-trip fix in `03_expressivity/decide/done.md` reports it as `LogicalType::BOOLEAN` to the model builder), but the generic branch only *skips* reset, which would leave a `DecidePropagateImpliedBounds` tightening in place instead of reverting it (using `is_binary` alone to gate the column reset silently widened the 0/1 box to `+inf` and turned the variable unbounded, regressing every boolean infeasible case — checking `is_boolean_var` first avoids both failure modes). The intrinsic-conflict guard must use `U < 0 AND L >= 0` (not just `U < 0`) so an explicitly-lowered floor (`x <= -1 AND x >= -5`) stays feasible.
...
## `SUM(MIN(expr)) PER col` rejected a multi-term inner MIN/MAX argument at bind time

**Broke**: A nested PER objective whose *outer* aggregate is `SUM` and whose inner `MIN`/`MAX` argument has more than one additive term was rejected in the binder before any model was built: `MAXIMIZE SUM(MIN((l.qty + 1) * x)) PER o.grp` failed with `Binder Error: Nested MIN() inside DECIDE expression must reference a DECIDE variable, found 'sum((x * (min(l.qty) + __MIN__)))'` — an error quoting syntax the user never wrote. `MAXIMIZE SUM(MIN(l.qty * x + x)) PER o.grp` and the `MAX`/`MINIMIZE` mirror failed the same way. The single-term inner (`SUM(MIN(l.qty * x))`) and the outer-`MIN`/`MAX`-instead-of-`SUM` forms (`MAX(MIN(...))`, `MIN(SUM(...))`) were unaffected.

**Cause**: `ToSymbolicRecursive` (`decide_symbolic.cpp`) represented `MIN(inner)`/`MAX(inner)` as a literal CAS product `Symbolic("__MIN__") * inner_symbolic`, mirroring how `SUM`/`AVG` are represented. SUM and AVG distribute over addition (`SUM(a+b) = SUM(a)+SUM(b)`), so a CAS auto-expanding the marker across a multi-term inner is harmless — the reconstruction in `FromSymbolicAggregateProduct` just sees several marked terms and rewraps them. MIN/MAX do **not** distribute: `NormalizeObjectiveRecursive`'s outer-`SUM` path (`decide_symbolic.cpp`, the only place a nested `SUM(...)` inner reaches the CAS) calls `.expand()` on the whole `SUM`'s inner, which distributed `__MIN__` across `(qty+1)*x`'s two terms *before* the marker could be reassociated with the whole product. The reconstruction then saw one distributed term and rebuilt `min(l.qty)` (data column only) plus a dangling bare `__MIN__` symbol added back in as if it were a column reference — hence the `min(l.qty) + __MIN__` in the error. The single-term case has nothing to distribute across, so it survived; the outer-`MIN`/`MAX` forms never call this `SUM`-only normalization path at all, so they were untouched.

**Fix/lesson**: Added `SymbolicTranslationContext::min_max_map` and stopped representing `MIN`/`MAX` as a marker product. `ToSymbolicRecursive` now wraps the *whole* `MIN(inner)`/`MAX(inner)` `FunctionExpression` as one opaque placeholder symbol (`__MINMAX_N__`), storing the original node in `min_max_map` — the same idiom `abs_map`/`data_map`/`subquery_map`/`column_map` already use — so the CAS never sees inside a MIN/MAX to distribute anything; `FromSymbolic` restores the node verbatim by copy. `SymbolicContainsDecideVariable` and `CollectDecideFactors` were taught to treat any `__MINMAX_` placeholder as decide-var-bearing (matching the existing `__ABS_` treatment), so a term built from one isn't dropped as a constant during SUM-term filtering. `FromSymbolicAggregateProduct` (and `IsMinMarker`/`IsMaxMarker`) were deleted as dead code — `SUM`/`AVG` still use the marker-product form since it's safe for them. The fix is general: it also covers the same shape on the constraint side (`SUM(MIN(...)) <= K`), which took the same `ToSymbolicRecursive` + `.expand()` path via `NormalizeComparisonExpr`'s default bypass and was equally broken, though not called out in the original repro. **Watch for**: any CAS round-trip that models a non-distributive construct (MIN/MAX, but also anything like a conditional or a non-linear op) as `MARKER * body` is only safe if the construct actually distributes over addition the way the marker's reassembly logic assumes — otherwise wrap the whole construct as an opaque placeholder instead, the same pattern used for ABS, subqueries, and qualified columns. Tests: `test/decide/tests/test_min_max_multiterm.py::test_per_sum_outer_min_inner_additive_coefficient`, `::test_per_sum_outer_min_inner_repeated_variable`, `::test_per_sum_outer_max_inner_additive_coefficient` (both `SUM(MIN(...))`/`SUM(MAX(...))` directions, oracle-verified). Discovered 2026-08-09 while writing regression coverage for the MIN/MAX linking-row fix above; fixed the same day.
