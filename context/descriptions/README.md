...
| `canonicalize.md`      | Multi-session plan to unify constraint canonicalization into a single pass: goal, canonical form, what has landed, phased remaining work, and open decisions | You are continuing the canonicalization refactor, or are about to change where a constraint's left/right shape is decided |
