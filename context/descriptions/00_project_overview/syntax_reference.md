...
- **Supported Operators**: `=`, `<`, `<=`, `>`, `>=`, `<>`.
  ...
  - `<` / `>` (strict): Rewritten internally to the integer-step form (`LHS < K` $\rightarrow$ `LHS <= ceil(K) - 1`), which is only valid when the LHS is provably integer-valued (every DECIDE variable is `INT`/`BOOL` and every coefficient is integral; bilinear products of integer-typed factors also count). Use `<=` / `>=` instead when it is not. The two ways it can fail are reported at different times:
    - A **`REAL` decision variable** anywhere in the comparison is rejected at bind time, before the query runs, with a message naming the variable and quoting the clause. The rule is stated on the declared type: `norm(e, 0, M) < K` on a `REAL` decision is refused as well, even though an L0 count is integral — write `<= K-1`, which is the same cap.
    - A **non-integer coefficient** over `INT`/`BOOL` decisions (`SUM(0.5 * x) < 5`, or a data column that evaluates to a fraction) can only be seen once coefficients are evaluated, and is rejected then with `InvalidInputException`.
