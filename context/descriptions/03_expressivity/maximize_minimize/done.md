...
**Both directions supported.** Easy (`MAXIMIZE` with `MIN`, `MINIMIZE` with `MAX`) uses the one-sided envelope pin alone; hard (`MAXIMIZE MAX`, `MINIMIZE MIN`) adds the per-row indicator layer (binary `y_i`, `SUM(y_i) >= 1`, Big-M link) so `z_k` is pinned to the true extreme rather than floating the objective to ±∞. Scalar multiplication (`2 * MIN(...)`) also works. Same mechanism and code (`EmitExtremumLink`, `ilp_linearization.cpp`) as composed constraints — see `../such_that/done.md` → "Composed MIN/MAX (both directions)".
