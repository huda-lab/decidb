...
from decidb import CSVLineTerminator
...
class TestReadCSV(object):
    def test_using_connection_wrapper(self):
        rel = decidb.read_csv(TestFile('category.csv'))
        res = rel.fetchone()
        print(res)
        assert res == (1, 'Action', datetime.datetime(2006, 2, 15, 4, 46, 27))

    def test_using_connection_wrapper_with_keyword(self):
        rel = decidb.read_csv(TestFile('category.csv'), dtype={'category_id': 'string'})
        res = rel.fetchone()
        print(res)
        assert res == ('1', 'Action', datetime.datetime(2006, 2, 15, 4, 46, 27))
    ...
    def test_delimiter_and_sep(self, duckdb_cursor):
        with pytest.raises(decidb.InvalidInputException, match="read_csv takes either 'delimiter' or 'sep', not both"):
    ...
    def test_compression_wrong(self, duckdb_cursor):
        with pytest.raises(decidb.Error, match="Input is not a GZIP stream"):
    ...
    def test_quote(self, duckdb_cursor):
        with pytest.raises(
            decidb.Error, match="The methods read_csv and read_csv_auto do not have the \"quote\" argument."
        ):
    ...
    def test_encoding_wrong(self, duckdb_cursor):
        with pytest.raises(
            decidb.BinderException, match="Copy is only supported for UTF-8 encoded files, ENCODING 'UTF-8'"
        ):
    ...
    def test_null_padding(self, duckdb_cursor):
        ...
        rel = decidb.read_csv(TestFile('nullpadding.csv'), null_padding=False, header=False)
        res = rel.fetchall()
        assert res == [
            ('# this file has a bunch of gunk at the top',),
            ('one,two,three,four',),
        ...
        rel = decidb.read_csv(TestFile('nullpadding.csv'), null_padding=True, header=False)
        res = rel.fetchall()
        assert res == [
            ('# this file has a bunch of gunk at the top', None, None, None),
            ('one', 'two', 'three', 'four'),
    ...
    def test_read_csv_combined(self, duckdb_cursor):
        ...
        rel = decidb.read_csv(CSV_FILE, skiprows=1, delimiter=",", quotechar='"', escapechar="\\", dtype=COLUMNS)
        res = rel.fetchall()
    ...
    def test_read_csv_names(self, tmp_path):
        ...
        with pytest.raises(decidb.InvalidInputException, match="read_csv only accepts 'names' as a list of strings"):
            rel = con.read_csv(file, names=True)

        with pytest.raises(decidb.InvalidInputException, match="not possible to detect the CSV Header"):
            rel = con.read_csv(file, names=['a', 'b', 'c', 'd', 'e'])

        # Duplicates are not okay
        with pytest.raises(decidb.BinderException, match="names must have unique values"):
    ...
    def test_read_csv_names_mixed_with_dtypes(self, tmp_path):
        ...
        with pytest.raises(decidb.BinderException, match=expected_error):
            rel = con.read_csv(
                file,
                names=['a', 'b', 'c'],
                dtype={
    ...
    def test_read_csv_empty_list(self):
        ...
        files = []
        with pytest.raises(
            decidb.InvalidInputException, match='Please provide a non-empty list of paths or file-like objects'
        ):
            rel = con.read_csv(files)
            res = rel.fetchall()
    ...
    def test_read_csv_list_invalid_path(self, tmp_path):
        ...
        with pytest.raises(decidb.IOException, match='No files found that match the pattern "not_valid_path"'):
            rel = con.read_csv(files)
            res = rel.fetchall()
    ...
    def test_strict_mode(self, tmp_path):
        ...
        con = decidb.connect()
        with pytest.raises(decidb.InvalidInputException, match="CSV Error on Line"):
            rel = con.read_csv(
                str(file1),
                header=True,
                delimiter='|',
