...
class TestDBConfig(object):
    ...
    def test_default_order(self, duckdb_cursor, pandas):
        ...
        con = decidb.connect(':memory:', config={'default_order': 'desc'})
    ...
    def test_null_order(self, duckdb_cursor, pandas):
        ...
        con = decidb.connect(':memory:', config={'default_null_order': 'nulls_last'})
        result = con.execute('select * from df order by a').fetchall()
    ...
    def test_multiple_options(self, duckdb_cursor, pandas):
        df = pandas.DataFrame({'a': [1, 2, 3, None]})
        con = decidb.connect(':memory:', config={'default_null_order': 'nulls_last', 'default_order': 'desc'})
        result = con.execute('select * from df order by a').fetchall()
    ...
    def test_external_access(self, duckdb_cursor, pandas):
        ...
        con_regular = decidb.connect(':memory:', config={})
        con_regular.execute('select * from df')
        # disable external access: this also disables pandas replacement scans
        con = decidb.connect(':memory:', config={'enable_external_access': False})
        # this should fail
        query_failed = False
        try:
    ...
    def test_extension_setting(self):
        ...
        if not repository:
            return
        con = decidb.connect(config={"TimeZone": "UTC", 'autoinstall_extension_repository': repository})
        assert 'UTC' == con.sql("select current_setting('TimeZone')").fetchone()[0]

    def test_unrecognized_option(self, duckdb_cursor):
        success = True
        try:
            con_regular = decidb.connect(':memory:', config={'thisoptionisprobablynotthere': '42'})
        except:
            success = False
    ...
    def test_incorrect_parameter(self, duckdb_cursor):
        success = True
        try:
            con_regular = decidb.connect(':memory:', config={'default_null_order': '42'})
        except:
            success = False
        assert success == False

    def test_user_agent_default(self, duckdb_cursor):
        con_regular = decidb.connect(':memory:')
    ...
    def test_user_agent_custom(self, duckdb_cursor):
        con_regular = decidb.connect(':memory:', config={'custom_user_agent': 'CUSTOM_STRING'})
    ...
    def test_secret_manager_option(self, duckdb_cursor):
        con = decidb.connect(':memory:', config={'allow_persistent_secrets': False})
