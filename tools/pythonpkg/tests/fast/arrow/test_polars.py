...
class TestPolars(object):
    ...
    def test_register_polars(self, duckdb_cursor):
        ...
        with pytest.raises(decidb.CatalogException, match='Table with name polars_df does not exist'):
    ...
    def test_empty_polars_dataframe(self, duckdb_cursor):
        ...
        with pytest.raises(
            decidb.InvalidInputException, match='Provided table/dataframe must have at least one column'
        ):
    ...
    def test_polars_from_json_error(self, duckdb_cursor):
        ...
        my_res = decidb.query("select my_str from my_table where my_str != 'y'")
