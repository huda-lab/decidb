...
def scoped_default(duckdb_cursor):
    default = decidb.connect(':default:')
    decidb.set_default_connection(duckdb_cursor)
    # Overwrite the default connection
    yield
    # Set it back on finalizing of the function
    decidb.set_default_connection(default)
...
class TestRAPIQuery(object):
    ...
    def test_query_non_select_result(self, duckdb_cursor):
        with pytest.raises(decidb.ParserException, match="syntax error"):
    ...
    def test_set_default_connection(self, scoped_default):
        decidb.sql("create table t as select 42")
        assert decidb.table('t').fetchall() == [(42,)]
        ...
        decidb.set_default_connection(con2)

        with pytest.raises(decidb.CatalogException, match='Table with name d does not exist'):
            con2.table('d').fetchall()

        assert decidb.table('t').fetchall() == [(21,)]

        decidb.sql("create table d as select [1,2,3]")

        assert decidb.table('d').fetchall() == [([1, 2, 3],)]
    ...
    def test_set_default_connection_error(self, scoped_default):
        with pytest.raises(TypeError, match='Invoked with: None'):
            ...
            decidb.set_default_connection(None)

        with pytest.raises(TypeError, match='Invoked with: 5'):
            decidb.set_default_connection(5)

        assert decidb.sql("select 42").fetchall() == [(42,)]
        decidb.close()

        with pytest.raises(decidb.ConnectionException, match='Connection Error: Connection already closed!'):
            decidb.sql("select 42").fetchall()
        ...
        con3 = decidb.connect()
        con3.close()
        decidb.set_default_connection(con3)
